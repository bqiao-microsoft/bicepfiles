...
function AttemptDeployMetadata($deploymentName, $resourceGroupName, $templateObject, $templateType) {
    $deploymentInfo = $null
    try {
...
function AttemptDeployment($path, $parameterFile, $deploymentName, $templateObject, $templateType) {
    ...
    While (($currentAttempt -lt $MaxRetries) -and (-not $isSuccess)) 
    {
        ...
        Try 
        {
            ...
            AttemptDeployMetadata $deploymentName $ResourceGroupName $templateObject $templateType
